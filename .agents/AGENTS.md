...
## 0.1 Ecosystem Awareness Contract

You are working on a single application project which is part of the larger **EdSentre 4-App Ecosystem**. The ecosystem consists of four interconnected applications sharing a single Supabase backend:
1. **Management App (`ed_sentre`)** — Windows Desktop/Web ERP for center admins.
2. **Student App (`ed_sentre_student`)** — Android/iOS Gamified exam and study portal.
3. **Teacher & Parent App (`ed_sentre_techer_and_parent`)** — Android/iOS Dual-Role class manager & progress tracker.
4. **Super Admin Dashboard (`ed_sentre_super_admin`)** — Windows Desktop platform administration dashboard.

Always remember:
- **Unified Vision**: You are only seeing one part of a unified, synchronized system. Do not write local features that violate cross-app workflows or RLS (Row Level Security) database isolation.
- **Consult Master Skill**: You must read and trigger the custom skill `edsentre_ecosystem` to understand the shared table schemas, RPC functions (`rpcCalculateTeacherSalary`, etc.), Edge Functions, and database contracts.
- **Shared Package**: Shared entities, DTOs, and constants reside in `edsentre_shared` (or shared packages). Never duplicate code or models.
- **Deprecate, Don't Break**: Never modify a shared RPC signature or database table schema directly without versioning it (e.g., `rpcCalculateTeacherSalaryV2`) to avoid breaking the other 3 apps.
