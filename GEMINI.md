...
# Contract Version: 4.1 | Last Reviewed: 2026 | Against: Claude 4.x + GPT-4o
...
- `provider` for new projects — use BLoC/Cubit.
...
7. **VALIDATE** — Does the fix break anything else?
...
|----------------|------------------|
| UI components used in 2+ places | `core/widgets/` |
...
[ ] All subscriptions, controllers, and nodes disposed
...
| `Stream<T>` | Multiple values over time. Use for: real-time data, WebSocket feeds, Supabase `stream()`, auth state changes. |
...
- Every `Stream`, `StreamController`, `AnimationController`, `TextEditingController`, `ScrollController`, `FocusNode`, and `PageController` must have a corresponding close/dispose call. No exceptions. Verified in code review checklist.
...
- This file applies to all Flutter/Dart projects owned by this developer.
...
**Validated Against**: Claude Opus 4.x, GPT-4o, Gemini 2.5 Pro
